...
class HOLO_dataset(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        if split == 'train':
            ...
            self.img_list = self.all_img_list[training_set_size:]
        else:
        ...
        # statistics
        self.mean = np.array([0.485, 0.456, 0.406], dtype=np.float32) # ziji todo
        self.std  = np.array([0.229, 0.224, 0.225], dtype=np.float32) # ziji todo
    ...
    def __getitem__(self, item):
        ...
        #  ============================   get labels   ==============================
        if self.split!='test':
            objects = self.get_label(img_name)
